...
namespace Application.Services
{
	public class ProductService : IProductService
	{
		...
		public async Task<ServiceResponse<IEnumerable<ProductDTO>>> GetProductsAsync(int? categoryId = null)
		{
			...
			try
			{
				...
				{
					data = data.Where(p => p.CategoryId == categoryId.Value).ToList();
				}
				...
				if (!createProduct.PrimaryDiamonds.IsNullOrEmpty())
				{
					foreach (var diamond in createProduct.PrimaryDiamonds)
					{
						if (diamond != 0)
						{
							...
							if (dia != null && (dia.Quantity - product.Quantity) < 0)
							{
								response.Success = false;
								response.Message = $"{dia.Name} not enough.";
								return response;
							}
							if (dia != null) dia.Quantity -= product.Quantity;
